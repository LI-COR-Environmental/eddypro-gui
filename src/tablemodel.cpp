...
int TableModel::colorHueDecStep_ = 1;

// FIXME: include angle checkbox with only 1 angle doesn't work
TableModel::TableModel(QObject *parent, QList<AngleItem> *angles)
...
QVariant TableModel::data(const QModelIndex &index, int role) const
{
    ...
    if (role == Qt::DisplayRole)
    {
        ...
        {
            case DEGREES:
                return QVariant(QString::number(item.angle_, 'f', 1) + tr(" [%1]").arg(Defs::DEGREE));
            default: Q_ASSERT(false);
        }
    }
    if (role == Qt::EditRole)
    ...
    if (angleSum() < MaxDegrees)
    {
        ...
        for (int i = 0; i < count; ++i)
        {
            angles_->insert(row,
                            ...
                            Qt::Checked,
                            Alia::getColor(colorHueDecStep_)));
        }
        endInsertRows();
        ++colorHueDecStep_;
        emit modified();
...
void TableModel::pruneEmptyRows()
{
    DEBUG_FUNC_NAME
...
double TableModel::angleToBeInserted()
{
...
void TableModel::setOffset(double alpha)
{
    DEBUG_FUNC_NAME
    qDebug() << "alpha" << alpha;
...
void TableModel::setSkipPruning(bool skip)
{
    DEBUG_FUNC_NAME
