    ...
    QTableView(parent)
{
    ...
    m_header->addSection(tr("Model"), tr("<b>Model:</b> Identify the model of the gas analyzer. For gas analyzers other than LI-COR, select the appropriate typology. OP and CP stand for open path and closed path, respectively."), CustomHeader::QuestionMark, ClickLabel::IrgaModel);
    m_header->addSection(tr("<i>Embedded software version</i>"), tr("<b>Embedded software version:</b> Identify the embedded software version that was running on the LI-7550 Analyzer Interface Unit at the time data were collected. Mandatory only for the LI-7200 and LI-7500A CO<sub>2</sub>/H<sub>2</sub>O analyzers."));
    ...
    m_header->addSection(tr("Longitudinal path length"), tr("<b>Longitudinal path length:</b> Path length in the direction of the optical source. Consult the analyzer's specifications or user manual."), CustomHeader::QuestionMark, ClickLabel::IrgaLPathLength);
    m_header->addSection(tr("Transversal path length"), tr("<b>Transversal path length:</b> Path length in the direction orthogonal to the optical source. Consult the analyzer's specifications or user manual."), CustomHeader::QuestionMark, ClickLabel::IrgaTPathLength);
...
{
    setViewportMargins(m_header->sizeHint().width() - 1, this->rowHeight(0) + 6, 0, 0);
